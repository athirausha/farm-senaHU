     ...
     if(validation_checker != true)
     {
           ...
           $.ajax({
             ...
             success: function( data ){
               console.log(data.data);
                ...
                $(".table_body").attr('id',data.data)
                $(".detailid").map(function(){return $(this).val('');})
                $('.rowadd_btn').prop('disabled',true);
      ...
      // to append table row
         $.ajax({
             ...
             url: "/get_row",
             async:false,
             data:{csrfmiddlewaretoken: window.CSRF_TOKEN, mx:$(".table_body").attr('id'),table_id:$("thead").attr('id'),rr:relation},
             success: function( data ){
               console.log(data.data);
             //   $('.table_body').append('<tr id="'+mx+'"><td>'+data.data[0].c_v+'<td><tr>')
             sr='<tr id="'+mx+'">'
                 for(i=0;i<data.data.length;i++)
                 {
                     sr=sr+'<td id="'+data.data[i].pk+'">'+data.data[i].c_v+'</td>'
                 }
                 sr=sr+'</tr>'
                 $('.table_body').append(sr)
             }
         });
      }
     else
...
// ADD DETAILS UPDATE KEEP MODAL OPEN
$('.detailid_add_new').click(function(){
 $('.rowadd_btn').prop('disabled',false);
...
// ADD DETAILS UPDATE KEEP MODAL OPEN END